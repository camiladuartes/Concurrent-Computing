...
public class MutexKNN implements IKNN {
	private int k;
	private double[][] testDataset, trainDataset;
	int NUM_INSTANCES_TRAIN, NUM_INSTANCES_TEST;
	private int accurates;
	private int NUM_THREADS;
	private ReentrantLock mutex;
	private Thread[] threads;
	
	public MutexKNN(int k, String TRAIN_FILENAME, String TEST_FILENAME, int NUM_THREADS, int NUM_INSTANCES_TRAIN, int NUM_INSTANCES_TEST) throws IOException {
		System.out.println("\n>> Mutex KNN Algorithm <<\n");
			...
			threads[i] = new Thread(new Runnable() {
				public void run() {
					for(int testLine = left; testLine < right; testLine++) {
						MutexKNN.this.knnPrediction(testLine);
					}
				}
			});
			threads[i].start();
